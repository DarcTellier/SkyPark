...
@export_category("Flip Move Settings")
@export var flip_degrees := 180.0
@export var flip_speed := 0.35
@export var move_distance := 128.0

@export_enum("Move Right", "Move Left")
var clockwise_move_direction := 0

@export_enum("Move Right", "Move Left")
var counterclockwise_move_direction := 1
...
func start_flip(direction: int) -> void:
	...
	if direction == 0:
		if clockwise_move_direction == 1:
			move_dir = -1.0
	else:
		if counterclockwise_move_direction == 1:
			move_dir = -1.0

	target_position = global_position + Vector2(move_distance * move_dir, 0)
